...
function EquipoCotizar() {
    ...
    // if($("#txt_equire_nom_1").val() == ""){
    //     alert("Ingrese un producto");
    //
    // }
    // else {
    //     length_re = tbody_re.length - 1;
    // }
    // if($("#txt_equinr_nom_1").val() == ""){
    //     alert("Ingrese un producto");
    //
    // }
    // else {
    //     length_nr = tbody_nr.length - 1;
    // }
        ...
        success: function resultado(data) {
            if(data == "0"){
                ...
                $.each(JSONobj, function (obj, item) {
                    $("#cmb_equire_nom_"+my_id).append('<option value="'+item.n_idproducto+'">'+item.v_nomproducto+' - '+item.v_modelo+'</option>');
                });
                ...
                $('#cmb_equire_nom_'+my_id+' option').click(function(){
                    ...
                    for (var i = 0; i < lenghtDatos; ++i) {
                        $.each(JSONobj[i], function (key, value) {
                            if (key==="n_idproducto" && String(value)===valor) {
                                $('#txt_equire_nom_'+my_id).val(JSONobj[i].v_nomproducto).removeClass("ocultar");
                                $('#spn_equire_cod_'+my_id).html(JSONobj[i].n_codigo);
                                $('#spn_equire_mod_'+my_id).html(JSONobj[i].v_modelo);
                                $('#spn_equire_mrc_'+my_id).html(JSONobj[i].v_marca);
                                $('#spn_equire_ume_'+my_id).html(JSONobj[i].v_nomumed);
                                $('#txt_equire_pid_'+my_id).val(JSONobj[i].n_idproducto);
...
function AgregarFilaRe(){
    ...
    //
    // var cloneCount = 1;
    // $("#btn_equipo_add").click(function(){
    //     $("#firstRowBody_equipo").clone().attr().append("#tbody_equipo");
    // });
